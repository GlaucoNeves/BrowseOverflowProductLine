...
static NSString *questionJSONold = @"{"
@"\"total\": 1,"
@"\"page\": 1,"
@"\"pagesize\": 30,"
@"\"questions\": ["
...
@"\"user_type\": \"registered\","
@"\"accept_rate\": 81,"
@"\"profile_image\": \"https://www.gravatar.com/avatar/a5b33ee3aa7798d88427679a16d7ad25?s=128&d=identicon&r=PG\","
@"\"display_name\": \"k06a\","
@"\"link\": \"http://stackoverflow.com/users/440168/k06a\""
@"},"
@"\"is_answered\": false,"
@"\"view_count\": 50,"
@"\"answer_count\": 1,"
@"\"score\": 2,"
@"\"last_activity_date\": 1411322503,"
@"\"creation_date\": 1410983072,"
@"\"last_edit_date\": 1411322503,"
@"\"question_id\": 25899051,"
@"\"link\": \"http://stackoverflow.com/questions/25899051/unable-to-call-specific-objective-c-class-method-in-swift\","
@"\"title\": \"Unable to call specific Objective-C class method in Swift\""
@"}"
@"]"
@"}";
...
- (void)testQuestionCreatedFromJSONHasPropertiesPresentedInJSON {
    XCTAssertEqual(question.questionID, 25899051, @"The question ID should match the data we sent");
    XCTAssertEqual([question.date timeIntervalSince1970], (NSTimeInterval)1410983072, @"The date of the question should match the data");
    XCTAssertEqualObjects(question.title, @"Unable to call specific Objective-C class method in Swift", @"Title should match the provided data");
    XCTAssertEqual(question.score, 2, @"Score should match the data");
    Person *asker = question.asker;
    XCTAssertEqualObjects(asker.name, @"k06a", @"Looks like I should have asked this question");
    //    XCTAssertEqualObjects([asker.avatarURL absoluteString], @"http://www.gravatar.com/avatar/563290c0c1b776a315b36e863b388a0c", @"The avatar URL should be based on the supplied email hash");
}
